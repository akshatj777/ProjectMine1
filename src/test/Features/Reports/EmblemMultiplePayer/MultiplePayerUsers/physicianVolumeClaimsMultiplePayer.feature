Feature: Verification of filters under Volume Claims report under Physicain for multiple payers

  Scenario Outline: Verification of participant,anchor facility,episode initiator,bpid and ccn filters under volume claims reports under physician
  ...
  Scenario Outline: User should be able to edit and delete the applied payer filter in volume claims report under physician
    Given I am on the login page
    When I enter email field <email> for login
    And I enter password field Testing1 for Login
    Then I click Access button
