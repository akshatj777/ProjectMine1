Feature: Verification of filters under Inpatient Episode Clearing report under Patient ID for multiple payers

  Scenario Outline: Verification of participant,anchor facility,episode initiator,bpid and ccn filters under inpatient episode clearing reports
  ...
  Scenario Outline: User should be able to edit and delete the applied payer filter in inpatient episode clearing report under patientid
    Given I am on the login page
    When I enter email field <email> for login
    And I enter password field Testing1 for Login
    Then I click Access button
