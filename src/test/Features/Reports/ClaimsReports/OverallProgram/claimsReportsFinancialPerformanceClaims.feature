Feature: Verification Claims Report For Financial Performance Claims Report
  ...
  Scenario Outline: Remedy awardee convener whose BPID starting with 2070 with model2 should see only not-applicable in fracture/non fracture filters when anchor admission quarter is < 2016Q4 in performance claims report under overall program
    Given I am on the login page
    When I enter email field <email> for login
    And I enter password field Testing1 for Login
    Then I click Access button
