Feature: Create Hospital org with multiple locations
  ...
  Scenario Outline:
    ...
    Given Select no Morg enter hospital data and click on submit  "<name>" and "<shortName>" and "<ccn>" and "<ein>" and "<npi>" and "<address1>" and "<address2>" and "<city>" and "<state>" and "<zip>" and "<locationName>" and "<locationType>" and "<marketId>" and "<regionId>" and "<locAddr1>" and "<locAddr2>" and "<locCity>" and "<locState>" and "<locZip>"
    And wait for the page to load and get the message
    Then org is created successfuly message verification "<expectedMsg>"

    Examples:

      | name              | shortName       | ccn        | ein        | npi        | address1    | address2  | city    | state | zip   | locationName                          | locationType | locAddr1                                | locAddr2                                | locCity                           | locState    | locZip                  | marketId | regionId | expectedMsg                                          |
      | HospNoMorgMulLocs | aptestShortName | 1234567890 | 1234567890 | 1234567890 | 256 5th ave | 7th floor | NewYork | NY    | 10020 | apLoc1Name,loc2Name,loc3Name,loc4Name | 8,9,3,4      | loc1Addr1,loc2addr1,loc3addr1,loc4addr1 | loc1addr2,loc2Addr2,loc3addr1,loc4addr2 | Bellemead,Fishkill,Cary,Beaverton | CT,FL,NJ,CT | 10020,08502,12345,56789 | 7,2,6,5  | 2,1,2,2  | Success! Hospital Organization Successfully Created. |