Feature: Edit Payor organization functionality tests
  ...
  Scenario Outline: <Description>
    ...
    Examples: 
      ...
      | To check the Allowed characters for the available fields                                    | PAYORNAME  | AllowedCharatcters   | !@$% ^&*()+}{;:<>?=-\\/.,Contact_person001    | !$%^&*'.-=+Contact_Email001@gmail.com         | 1(235)526 5236 | !@$test% ^&2356*()_+}{;:<>?=-\\/.,Address1              | !@$test% ^&2356*()_+}{;:<>?=-\\/.,Address2              | !@$test% ^&2356*()_+}{;:<>?=-\\/.,City        | New York |       10001 | Payor Organization Successfully Updated. |
