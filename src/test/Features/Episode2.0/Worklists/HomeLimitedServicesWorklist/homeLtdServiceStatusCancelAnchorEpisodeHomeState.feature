Feature: While patient in worklist, Episode state changes
    ...
    Then I select the "9" LOS days on Discharge date on Add Transition
    ...
    Scenario Outline: Verify Patient should not be included in Home With Limited Services worklist when Cancel or complete the anchor episode while patient is in th HOM with Limited services.
