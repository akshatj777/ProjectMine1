Feature: Patient status on Other/Alternative Work List
  ...
  Scenario Outline: M2 Active episode patient discharge to Other - With All Care types
    ...
    Then I select the Discharge care setting value "OTHER - Other" on add a new transition
    Then I wait for 4000 milli seconds
    Then I select the Discharge care type value "<Care Type>" on add a new transition
    Then I wait for 5000 milli seconds
    Then I select the Discharge facility value "Brothers Medical Center Inc" on add a new transition
    Then I wait for 5000 milli seconds
