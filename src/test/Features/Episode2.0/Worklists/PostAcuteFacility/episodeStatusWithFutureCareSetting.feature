...
Scenario Outline: Patient has Active episode status with current care setting is future one
    ...
    Then I click on datepicker button to select the admit date on add a new transition
    Then I click on the centre of the calendar header to select date and month on Transition Page
    Then I click on the previous next link to select the required year "-31" on date picker
    Then I select the month "-31" from calendar from date picker
    Then I select the "-31" from the calendar from date picker on Transition Page
    Then I select the "-31" time from the calendar from date picker on Transition Page
    Then I wait for 5000 milli seconds
    Then I select the care setting value "LTC - Long-Term Care Hospital" on add a new transition
    Then I wait for 5000 milli seconds
    Then I select the facility value "Madonna Rehabilitation Ltc Hospital" on add a new transition
