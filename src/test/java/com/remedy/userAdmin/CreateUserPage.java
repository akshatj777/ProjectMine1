...
public class CreateUserPage extends BaseClass {
	...
	public void selectOrganizationalRole(String desc) throws InterruptedException {
		...
		if (isElementVisible(element) == true) {
			element.click();
			userRole = desc;
	...
	public void iEnterNPI(String npi, String role) {
		if (role.equals("Physicians")) {
			if (npi.equals("NPI")) {
				...
				iFillInText(driver.findElement(By.xpath("//input[@placeholder='NPI']")),
	...
	public void iClickHealthSystemField() {
		delay();
		boolean value = isElementPresentOnPage(
				By.xpath("//div[@name='participantSelect']/div[@placeholder='Search']/span"));
		if (value == true) {
			clickElement(
					driver.findElement(By.xpath("//div[@name='participantSelect']/div[@placeholder='Search']/span")));
		}
	}
	...
	public void iEnterHealthSystemSerachText(String text) throws InterruptedException {
		if (!(text.equals(""))) {
			...
			driver.findElement(By.xpath("//div[text()='Select']/parent::div/following-sibling::div/div/div/input"))
					.sendKeys(text);
			Thread.sleep(2000);
		}
	...
	public void iClickOnEpisodesTwoTileUnderSpecificUserLoginPage(String role) {
		String application = CreateUserPage.usersApplicationsPerRole.get(role)
				.get(role.substring((role.indexOf("-") + 1)));
	...
	public void iClickOnAddNoteAndVerifyRole(String userrole, String role) throws InterruptedException {
		...
		while (st.hasMoreTokens()) {
			if (st.nextToken().trim().equals("Episodes")) {
				...
				longDelay();
				scrollIntoViewByJS(driver.findElement(By
						.xpath("html/body/div[4]/div[2]/div/div[2]/div[4]/div/div/span/div[1]/div[2]/div[4]/a[1]/i")));
				driver.findElement(
						By.xpath("html/body/div[4]/div[2]/div/div[2]/div[4]/div/div/span/div[1]/div[2]/div[4]/a[1]/i"))
						.click();
				delay();
				...
				// driver.findElements(By.xpath("//a[@class='btn btn-default
				// dropdown-toggle']/i[@class='fa fa-cog']")).get(0).click();
				// delay();
				// //driver.findElements(By.xpath("//a[contains(text(),'Add
				// Note')]")).get(0).click();
				// scrollIntoViewByJS(driver.findElement(By.xpath("//div[contains(@class,'center
				// open')]//a[@symfony-routing='new_note']")));
				// driver.findElement(By.xpath("//div[contains(@class,'center
				// open')]//a[@symfony-routing='new_note']")).click();
				// Thread.sleep(2000);
	...
	public void iVerifyNavigationOnInstituteHomePage(String role) {
		...
		new WebDriverWait(driver, 180).until(webDriver -> ((JavascriptExecutor) webDriver)
				.executeScript("return document.readyState").equals("complete"));
		new WebDriverWait(driver, 180).until(ExpectedConditions.visibilityOfElementLocated(
				By.xpath("//a[@href='http://02-wpress-qa-ue1a.remedypartners.com/index.php/contact-us/']")));
	...
	public void iVerifyNavigationOnRemedyUHomePage(String role) {
		...
		if (application.contains("Lessons")) {
			...
			if (user.equalsIgnoreCase("Remedy Technical Administrator")
					|| user.equalsIgnoreCase("Partner Program Administrator")
					|| user.equalsIgnoreCase("Remedy Program Administrator")
	...
	public void clickOnFieldButton(String fieldName) {
		if (fieldName.equalsIgnoreCase("Role")) {
			...
			scrollIntoViewByJS(driver.findElement(By.xpath("//input[@placeholder='" + fieldName + "']")));
			iWillWaitToSee(By.xpath("//input[@placeholder='" + fieldName + "']"));
	...
	public void clickSubmitButtonForDifferentUsers(String user) throws Throwable {
		...
		wait.until(ExpectedConditions.invisibilityOfElementLocated(
				By.cssSelector(".ui.modal.transition.visible.active.component-add-user-form")));
	...
	public void verifyAppUnchecked(String fieldName) throws Throwable {
		...
		while (st.hasMoreTokens()) {
			Assert.assertTrue(isElementPresent(
					By.xpath("//label[text()='" + st.nextToken().trim() + "']/parent::div[@class='ui checkbox']")));
	...
	public void verifyAppChecked(String fieldName) throws Throwable {
		...
		while (st.hasMoreTokens()) {
			Assert.assertTrue(isElementPresent(By.xpath(
					"//label[text()='" + st.nextToken().trim() + "']/parent::div[@class='ui checked checkbox']")));
		}
	}
	...
	public void verifyLearningPathwayAvailable() throws Throwable {
		Assert.assertTrue(
				isElementPresentOnPage(By.cssSelector(".column.padding>.component-learning-pathway-dropdown")));
	}
		...
		if (!(searchParam.equals(""))) {
			iFillInText(
					driver.findElement(By
							.xpath("//div[@class='select-checkbox-dropdown-menu menu']//input[@placeholder='Search']")),
					searchParam);
		}
	}
	...
	public void deselectPrograms(String programList) throws Throwable {
		if (!(programList.equals(""))) {
			...
			// {
			// longDelay();
			// driver.findElement(By.xpath("//i[@class='delete
			// icon']")).click();
			// longDelay();
			// }
			if (programList.contains(",")) {
		...
		if (!(locationList.equals(""))) {
			if (locationList.equalsIgnoreCase("all locations")) {
				...
				while (st.hasMoreTokens()) {
					...
					driver.findElement(By
							.xpath("//p[text()='Which location(s) does this user have access to?']/..//input[@placeholder='Search']"))
							.clear();
					iFillInText(
							driver.findElement(By
									.xpath("//p[text()='Which location(s) does this user have access to?']/..//input[@placeholder='Search']")),
							location);
					String a = "//th[text()='" + BPID
							+ "']/parent::tr/parent::thead/parent::table//label[contains(text(),\"" + location + "\")]";
					iWillWaitToSee(By.xpath("//th[text()='" + BPID
							+ "']/parent::tr/parent::thead/parent::table//label[contains(text(),\"" + location
							+ "\")]"));
					driver.findElement(By.xpath("//th[text()='" + BPID
							+ "']/parent::tr/parent::thead/parent::table//label[contains(text(),\"" + location
							+ "\")]")).click();
					Thread.sleep(3000);
				}
		...
		if (!(locationList.equals(""))) {
			if (locationList.equalsIgnoreCase("all locations")) {
				clickElement(driver.findElement(By.xpath(
						"html/body/div[2]//form/section//div[4]/section/section/div[2]/div/div[1]/div[1]/label")));
				delay();
			} else if (locationList.contains(",")) {
				StringTokenizer st = new StringTokenizer(locationList, ",");
				while (st.hasMoreTokens()) {
					...
					String BPID = token.substring(0, token.indexOf("--"));
					driver.findElement(By
							.xpath("html/body/div[2]/div/div[2]/div[1]/form/section/div/div/div/div/div[4]/section/section/div[1]/div/input"))
							.clear();
					iFillInText(
							driver.findElement(By
									.xpath("html/body/div[2]/div/div[2]/div[1]/form/section/div/div/div/div/div[4]/section/section/div[1]/div/input")),
							location);
					iWillWaitToSee(By
							.xpath("html/body/div[2]//form/section//div[4]/section/section/div[2]//table[@class='ui table component-bpid-table']//th[text()='"
									+ BPID + "']/../../../tbody/tr[2]/td[1]/div/label[contains(text(),\"" + location
									+ "\")]"));
					driver.findElement(By
							.xpath("html/body/div[2]//form/section//div[4]/section/section/div[2]//table[@class='ui table component-bpid-table']//th[text()='"
									+ BPID + "']/../../../tbody/tr[2]/td[1]/div/label[contains(text(),\"" + location
		...
		if (!(locationList.equals(""))) {
			if (locationList.equalsIgnoreCase("all locations")) {
				...
				while (st.hasMoreTokens()) {
					...
					String BPID = token.substring(0, token.indexOf("--"));
					driver.findElement(By
							.xpath("html/body/div[2]/div/div[2]/div[1]/form/section/div/div/div/div/div[6]/section/section/div[1]/div/input"))
							.clear();
					iFillInText(
							driver.findElement(By
									.xpath("html/body/div[2]/div/div[2]/div[1]/form/section/div/div/div/div/div[6]/section/section/div[1]/div/input")),
							location);
					iWillWaitToSee(By
							.xpath("html/body/div[2]//form/section//div[6]/section/section/div[2]//table[@class='ui table component-bpid-table']//th[text()='"
									+ BPID + "']/../../../tbody/tr[2]/td[1]/div/label[contains(text(),\"" + location
									+ "\")]"));
					driver.findElement(By
							.xpath("html/body/div[2]//form/section//div[6]/section/section/div[2]//table[@class='ui table component-bpid-table']//th[text()='"
	...
	public void verifyDefaultProgramOrganization(String programName) throws Throwable {
		...
		Assert.assertTrue(isElementPresentOnPage(
				By.xpath("//label[text()='" + programName + "']/parent::div[@class='ui checked checkbox']")));
	}
	...
	public void verifyTextPresentInLocationSearchLabel(String text) throws Throwable {
		...
		String Actual = driver
				.findElements(
						By.xpath("//section[@class='component-remedy-facility-select']//div[@class='ui checkbox']"))
				.get(1).getText();
	...
	public void iVerifyValueIsNotPresentInProductDropDownMenu(String products) {
		...
		List<WebElement> listItems = driver.findElements(
				By.xpath("//menu-dropdown[@class='flex-item order-0']//a[contains(@class,'btn btn-flyout-nav')]"));
		while (st.hasMoreTokens()) {
			String newToken = st.nextToken();
			if (newToken.contains("Lessons")) {
				token = "RemedyU";
	...
	public void iVerifyDropDownValueFromProfileIcon(String text, String role) {
		...
		if (text.equals("Internal Support")) {
			Assert.assertTrue(isElementPresentOnPage(
					By.xpath("//a[@ng-href='https://jira.remedypartners.com/servicedesk/customer/portal/2']")));
			driver.get("https://cdn-qa.remedypartners.com/");
			delay();
		} else if (text.equals("Support")) {
			if (!(role.equals("Remedy Sales Team") || role.equals("Prospective Partner Executive"))) {
				Assert.assertTrue(isElementPresentOnPage(By.xpath(
						"//a[@ng-href='http://jira.remedysystems.com/servicedesk/customer/portal/2/user/login?destination=portal%2F2']")));
				driver.get("https://cdn-qa.remedypartners.com/");
				delay();
			}
	...
	public void iclicktofieldfilterunderfilterfieldforuser(String filterField, String filterTitle, String role)
			throws Throwable {
		String application = CreateUserPage.usersApplicationsPerRole.get(role)
				.get(role.substring((role.indexOf("-") + 1)));
		StringTokenizer st = new StringTokenizer(application, ",");
		while (st.hasMoreTokens()) {
			if (st.nextToken().trim().equals("Reports")) {
				iWillWaitToSee(By.xpath(".//*[@id='fieldListTreeContent']//div[@formula='[" + filterTitle + "].["
						+ filterField + "]']"));
				clickElement(driver.findElement(By.xpath(".//*[@id='fieldListTreeContent']//div[@formula='["
						+ filterTitle + "].[" + filterField + "]']")));
				iWillWaitToSee(By.xpath(".//*[@id='fieldListTreeContent']//div[@formula='[" + filterTitle + "].["
						+ filterField + "]']/div"));
				clickElement(driver.findElement(By.xpath(".//*[@id='fieldListTreeContent']//div[@formula='["
						+ filterTitle + "].[" + filterField + "]']/div")));
			}
		}
	}
	...
	public void ishouldseeinthefiltervaluelistforuser(String text, String role) throws Throwable {
		...
		while (st.hasMoreTokens()) {
			if (st.nextToken().trim().equals("Reports")) {
				verifyTextForElement(driver.findElement(By.xpath("//*[@id='dialogTitleBar']/table/tbody/tr/td[1]")),
						"Filter on BPID");
